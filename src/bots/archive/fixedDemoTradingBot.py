...
class DemoTradingBot:
    ...
    def _handle_market_data_update(self, data: MarketDataResponse):
        ...
        #now compare put-call parity price with futures
        #print(data)
        #print(self._instrument_ids)
        for instr in self._instrument_ids:
            print(f"Instrument {instr} at time {data.time}")
            time = data.time/1000
            #future<=>strike+(call-put)
            exp = int((time//15))*15+45#expiry date to watch
            call={}
            put={}
            future=0#weighted average
            call_count={}
            put_count={}
            future_count=0
            """
            try:
                print(self.assets[instr]["future"][f"{instr}_future_{exp}"])
                print(self.assets[instr]["call"])
                print(self.assets[instr]["put"])
            except KeyError:
                continue
            """
            for book in self.assets[instr]["future"][f"{instr}_future_{exp}"].values():#book is bid/ask
                for key in book.keys():#key is price
                    future_count+=book[key]
            for book in self.assets[instr]["future"][f"{instr}_future_{exp}"].values():#book is bid/ask
                for key in book.keys():#key is price
                    future+=(book[key]/future_count) * int(key)
            print(future)
            for pos in self.assets[instr]["call"].keys():
                if int(pos.split("_")[3]) == exp:
                    if not(pos.split("_")[2] in call_count.values()):
                        call_count[pos.split("_")[2]]=0
                        call[pos.split("_")[2]]=0
                    for book in self.assets[instr]["call"][pos].values():#book is  bid/ask
                        for key in book.keys():
                            call_count[pos.split("_")[2]]+=book[key]
            for pos in self.assets[instr]["call"].keys():
                if int(pos.split("_")[3]) == exp:
                    for book in self.assets[instr]["call"][pos].values():  # book is  bid/ask
                        for key in book.keys():
                            call[pos.split("_")[2]] += (book[key]/call_count[pos.split("_")[2]]) * int(key)
            print(call)
            for pos in self.assets[instr]["put"].keys():
                if int(pos.split("_")[3]) == exp:
                    if not (pos.split("_")[2] in put_count.values()):
                        put_count[pos.split("_")[2]] = 0
                        put[pos.split("_")[2]] = 0
                    for book in self.assets[instr]["put"][pos].values():  # book is bid/ask
                        for key in book.keys():
                            put_count[pos.split("_")[2]] += book[key]
            for pos in self.assets[instr]["put"].keys():
                if int(pos.split("_")[3]) == exp:
                    for book in self.assets[instr]["put"][pos].values():  # book is bid/ask
                        for key in book.keys():
                            put[pos.split("_")[2]] += (book[key] / put_count[pos.split("_")[2]]) * int(key)
            print(put)

            threshold = 10  # Minimum profit edge needed (in price units)

            for strike_str in call.keys():
                strike = int(strike_str)  # ensure it's an integer for math
                call_price = call[strike_str]
                put_price = put[strike_str]

                # Synthetic future = Call - Put + Strike
                synthetic_future = call_price - put_price + strike

                # Check if synthetic future is cheaper than real one
                if synthetic_future > future * 1.0225:
                    print(f"[+] Arbitrage Opportunity at strike {strike}")
                    print(f"    Real future:     {future:.2f}")
                    print(f"    Synthetic future:{synthetic_future:.2f}")
                    print(f"    Difference:      {future - synthetic_future:.2f}")
                    # Example action
                    if not self._trade_sequence_triggered:
                        self._trade_sequence_triggered = True
                        self._instrument_ids.add(instr)
                        asyncio.create_task(self.trade(instr, strike, exp, future, call_price, put_price))
                    # place_order("BUY", "CALL", strike)
                    #self.buy(f"{instr}_call_{strike}_{exp}", 1000)
                    # place_order("SELL", "PUT", strike)
                    #self.sell(f"{instr}_put_{strike}_{exp}", 1000)
                    # place_order("SELL", "FUTURE")
                    #self.sell(f"{instr}_future_{exp}", 1000)

            # compare futures and put call parity
            #print(instr, depths)
            """
    ...
    async def trade(self, instr: InstrumentID_t, strike: Price_t, exp: Price_t, futurePrice, callPrice, putPrice):
        self._trade_sequence_triggered = True
        print("BUY FUTURE")
        print(f"{instr}_future_{exp}")
        resp3 = await self.buy(f"{instr}_future_{exp}", futurePrice)
        print(resp3)
        if not self._is_success(resp3):
            print("Failed to BUY FUTURE.")
            self._trade_sequence_triggered = False
            return

        print("BUY PUT")
        print(f"{instr}_put_{strike}_{exp}")
        resp2 = await self.buy(f"{instr}_put_{strike}_{exp}", putPrice)
        print(resp2)
        if not self._is_success(resp2):
            print("Failed to BUY PUT. Aborting sequence.")
            self._trade_sequence_triggered = False
            return
